...
static void fcu_firmware_transfer(void)
{
...
unsigned char e2_blank_check(unsigned short block_adress)
{
	...
	*e2_FLASH							= 0x71;		//ブランクチェック第一サイクルロックビットリードモードに移行
	e2_FLASH[block_adress]				= 0xd0;			//ブランクチェック第二サイクルブランクチェックしたいアドレスにD0h書き込み
...
unsigned char e2data_erase(unsigned short erase_address)
{
	...
	e2_FLASH[erase_address]				= 0xd0;		//ブロックイレーズ第二サイクル消去したいアドレスにD0h書き込み
	return fcu_wait(250);
}
/*
 * データフラッシュのデータを全部消去する
